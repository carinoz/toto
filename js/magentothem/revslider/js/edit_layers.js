	...
	**************************************/
		...
		jQuery("#layeranimeditor_wrap").dialog({
			...
			buttons:{
				"Save/Change":function(){
					...
					console.log(1221); console.log(animObj);
								...
								jQuery("#dialog-change-layeranimation-save-as").dialog({
									...
									buttons: {
										'Save as new': function(){
											...
											var update = true;
											console.log(1238); console.log(jQuery('input[name="layeranimation_save_as"]').val());
											if(id !== false){
												update = false;
	...
	var checkIfAnimExists = function(handle){
		...
		if(typeof initLayerAnims === 'object' && !jQuery.isEmptyObject(initLayerAnims)){
			for(var key in initLayerAnims){
				console.log(1556);
				console.log(initLayerAnims[key]['handle']);
	...
	var updateAnimInDb = function(handle, animObj, id){
		...
		var animation_url = jQuery('#animation_url').val();
			if(JSON && JSON.stringify)
				animObj = JSON.stringify(animObj);
					console.log(animObj); 
			  new Ajax.Request(animation_url, {
					 parameters: {anim_data:animObj},
					 method:'post',
    ...
    t.newLayerImage = function () {
        if (jQuery('#image_url_upload').val() != '') {
            var mediaUrl = jQuery('#image_url_upload').attr('media_url');
            var urlImage = mediaUrl+jQuery('#image_url_upload').val();
